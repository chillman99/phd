...
public class ReduceISOPeaks_hashset {
	...
	Set<PointThreeD> threedDpointsH = new HashSet<PointThreeD>(threedDpoints);

	int outerPos = 0;
	for (PointThreeD outerLoop : threedDpointsH ) {
		int innerPos = 0;
		outerWPM=outerLoop.getWpm();
		outerSUMI=outerLoop.getSumI();
		outerMAXRT=outerLoop.getMaxRT();
		outerMINRT=outerLoop.getMinRT();
		outerCharge=outerLoop.getCharge();
 
		for (PointThreeD innerLoop : threedDpointsH) {
			innerWPM=innerLoop.getWpm();
			innerSUMI=innerLoop.getSumI();
			innerRT=innerLoop.getWpRT();
			innerCharge=innerLoop.getCharge();
			...
			//Match on Intensity			
			 ...
			 //Enhanced method similar using Standard Deviation of weighted 3d peak
				   ...
				   if( match >= 1) {
					   ...
					   innerCurve = innerLoop.getCurveID();
					   outerCurve = outerLoop.getCurveID();
